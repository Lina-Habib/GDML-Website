...
<div class="container mx-auto p-6 max-w-2xl">
    <div class="bg-blue-100 shadow-lg rounded-lg p-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-8 text-center">إضافة معرض صور جديد</h1>
        <form action="{{ route('images.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                <label for="images" class="block text-gray-700 font-medium text-lg mb-3">الصور (يمكنك اختيار عدة صور)</label>
                <input type="file" class="w-full border rounded-lg px-4 py-3 text-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('images.*') border-red-500 @enderror" id="images" name="images[]" accept="image/*" multiple required>
                @error('images.*')
                    <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                @enderror
            </div>
